'use client';

import React from 'react';
import { Client } from '@/types/crm';

interface WhatsAppButtonProps {
  client: Client;
  size?: 'sm' | 'md' | 'icon';
  variant?: 'outline' | 'solid' | 'badge';
}

export const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({ 
  client, 
  size = 'sm', 
  variant = 'solid' 
}) => {
  const generateMessage = () => {
    const appraiserName = 'Mahesh Bhaskara (Certified Lead Appraiser, Shree QA Solutions)';
    const company = client.name;
    const service = client.service_type;

    if (client.stage === 'renewal_due') {
      const expiryText = client.cert_expiry_date ? ` due on ${client.cert_expiry_date}` : '';
      return `Dear Team ${company},\n\nGreetings from ${appraiserName}.\n\nThis is a friendly reminder regarding your upcoming ${service} certification renewal${expiryText}. We would like to coordinate the recertification schedule and audit plan.\n\nPlease let us know a suitable time for a quick discussion.\n\nWarm regards,\nShree QA Solutions (Kukatpally, Hyderabad)\nwww.shreeqasolutions.com`;
    }

    if (client.stage === 'in_appraisal') {
      const substage = client.pipeline_substage ? ` [${client.pipeline_substage.replace('_', ' ').toUpperCase()}]` : '';
      return `Hello Team ${company},\n\nThis is ${appraiserName} following up on your ${service} appraisal progress${substage}.\n\nPlease let us know if the team requires any assistance with process artifact submission or ATM readiness verification.\n\nBest regards,\nShree QA Solutions`;
    }

    if (client.stage === 'lead') {
      return `Dear ${company} Team,\n\nGreetings from ${appraiserName}!\n\nThank you for reaching out to Shree QA Solutions regarding ${service} certification lead appraisal. We would be pleased to schedule an initial scoping and readiness assessment session.\n\nLooking forward to collaborating with your team.\n\nWebsite: www.shreeqasolutions.com`;
    }

    return `Hello Team ${company},\n\nGreetings from ${appraiserName} at Shree QA Solutions regarding your ${service} compliance engagement.\n\nPlease feel free to reach out for any audit or appraisal assistance.\n\nContact: +91 9177020007 | www.shreeqasolutions.com`;
  };

  const handleWhatsAppClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    const message = generateMessage();
    const encoded = encodeURIComponent(message);
    const url = `https://api.whatsapp.com/send?text=${encoded}`;
    window.open(url, '_blank', 'noopener,noreferrer');
  };

  const WhatsAppIcon = () => (
    <svg 
      viewBox="0 0 24 24" 
      className={size === 'icon' ? 'w-4 h-4' : 'w-3.5 h-3.5'} 
      fill="currentColor"
    >
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.886-9.888 9.886m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
    </svg>
  );

  if (size === 'icon') {
    return (
      <button
        type="button"
        onClick={handleWhatsAppClick}
        title={`Chat with ${client.name} via WhatsApp`}
        className="p-1.5 rounded-md bg-[#25D366]/10 text-[#128C7E] hover:bg-[#25D366] hover:text-white transition-colors flex items-center justify-center"
      >
        <WhatsAppIcon />
      </button>
    );
  }

  return (
    <button
      type="button"
      onClick={handleWhatsAppClick}
      title={`Open WhatsApp template for ${client.name}`}
      className={`inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md text-xs font-semibold transition-all ${
        variant === 'solid'
          ? 'bg-[#25D366] text-white hover:bg-[#1EBE5D] shadow-xs'
          : 'bg-[#25D366]/10 text-[#128C7E] border border-[#25D366]/30 hover:bg-[#25D366] hover:text-white'
      }`}
    >
      <WhatsAppIcon />
      <span>WhatsApp</span>
    </button>
  );
};
